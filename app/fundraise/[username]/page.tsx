...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const fundraiser: any = process.env.KV_URL ? await kv.get(`fundraise:${username.toLowerCase()}`) : null;
...
export default async function FundraisePage({ params }: Props) {
  ...
  if (process.env.KV_URL) {
    fundraiser = await kv.get(`fundraise:${lowerUsername}`);
    supporters = (await kv.get(`fundraise:${lowerUsername}:supporters`)) || [];
